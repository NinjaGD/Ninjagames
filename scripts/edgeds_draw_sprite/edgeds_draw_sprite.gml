/// @description edgeds_draw_sprite(sprite, index, x, y, rot, color, alpha, scale mode, notme, xscale, yscale);
...
if global.display_xscale != 0 and global.display_yscale != 0 {
   ...
   if argument7 == 3 {
      //Width
      sprite_xscale = clamp(display_width/(((argument2 - __view_get( e__VW.XView, global.display_view ))*0.5) + sprite_get_width(argument0)), 0, 1);
      sprite_yscale = sprite_xscale;

      //Height
      if (argument3 + sprite_get_height(argument0)) > display_height {
         sprite_yscale = sprite_xscale*clamp(display_height/(((argument3 - __view_get( e__VW.YView, global.display_view ))*0.5) + sprite_get_height(argument0)), 0, 1);
         sprite_xscale = sprite_yscale;
      }
   }
